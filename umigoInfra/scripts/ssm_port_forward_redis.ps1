...
if (-not $RemoteHost) {
  $RemoteHost = Get-TfOutputRaw 'redis_host'
}

if (-not $RemoteHost) {
  throw "Remote host not provided and Terraform output 'redis_host' could not be resolved. Pass -RemoteHost."
}

if (-not $RemotePort) {
  $RemotePort = Get-TfOutputRaw 'redis_port'
}
